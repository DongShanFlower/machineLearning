...
def spamTest():
    ...
    for i in range(1, 26):
        # 切分，解析数据，并归类为 1 类别
        wordList = textParse(open('C:/PythonWorkSp/AiLearning-master/data/4.NaiveBayes/email/spam/%d.txt' % i).read())
        docList.append(wordList)
        classList.append(1)
        # 切分，解析数据，并归类为 0 类别
        try:
            wordList = textParse(open('C:/PythonWorkSp/AiLearning-master/data/4.NaiveBayes/email/ham/%d.txt' % i).read())
...
def testParseTest():
    print(textParse(open('C:/PythonWorkSp/AiLearning-master/data/4.NaiveBayes/email/ham/1.txt').read()))
